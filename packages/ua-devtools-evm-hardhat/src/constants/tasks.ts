export const TASK_LZ_ERRORS_DECODE = 'lz:errors:decode'
export const TASK_LZ_ERRORS_LIST = 'lz:errors:list'

export const TASK_LZ_OAPP_WIRE = 'lz:oapp:wire'
export const TASK_LZ_OAPP_CONFIG_GET_DEFAULT = 'lz:oapp:config:get:default'
export const TASK_LZ_OAPP_CONFIG_GET_EXECUTOR = 'lz:oapp:config:get:executor'
export const TASK_LZ_OAPP_CONFIG_GET_READ = 'lz:oapp:config:get:read'
export const TASK_LZ_OAPP_CONFIG_GET = 'lz:oapp:config:get'
export const TASK_LZ_OAPP_PEERS_GET = 'lz:oapp:peers:get'
export const TASK_LZ_OAPP_ENFORCED_OPTS_GET = 'lz:oapp:enforced-opts:get'
export const TASK_LZ_OAPP_CONFIG_INIT = 'lz:oapp:config:init'

export const SUBTASK_LZ_OAPP_WIRE_CONFIGURE = '::lz:oapp:wire:configure'
export const SUBTASK_LZ_OAPP_READ_WIRE_CONFIGURE = '::lz:oapp-read:wire:configure'
export const SUBTASK_LZ_OAPP_CONFIG_LOAD = '::lz:oapp:config:load'

export const TASK_LZ_OWNABLE_TRANSFER_OWNERSHIP = 'lz:ownable:transfer-ownership'

export const TASK_LZ_READ_RESOLVE_COMMAND = 'lz:read:resolve-command'
export const TASK_LZ_OAPP_READ_WIRE = 'lz:oapp-read:wire'
export const TASK_LZ_OAPP_READ_CONFIG_GET = 'lz:oapp-read:config:get'
export const TASK_LZ_OAPP_READ_CONFIG_INIT = 'lz:oapp-read:config:init'
export const SUBTASK_LZ_OAPP_READ_WIRE_CONFIGURE = '::lz:oapp-read:wire:configure'
export const TASK_LZ_OAPP_READ_CONFIG_GET_CHANNEL = 'lz:oapp-read:config:get:channel'
