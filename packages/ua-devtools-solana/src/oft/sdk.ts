...
import { accounts, oft, SetOFTConfigParams } from '@layerzerolabs/oft-v2-solana-sdk'
import {
    type OmniAddress,
...
export class OFT extends OmniSDK implements IOApp {
    ...
    protected async getSetOftConfigIxAccounts(): Promise<{ oftStore: UmiPublicKey; admin: Signer }> {
        return {
    ...
    protected async generateSetOftConfigIx(
        params: SetOFTConfigParams,
        programs: { oft: UmiPublicKey; endpoint?: UmiPublicKey }
    ): Promise<WrappedInstruction> {
        const accounts = await this.getSetOftConfigIxAccounts()
        return oft.setOFTConfig(accounts, params, programs)
    }
