...
describe('msgLib-tests', () => {
    ...
    describe('get default executor config', () => {
        it.only('Should get default executor config', async () => {
            const executorConfig = await msgLib.get_default_executor_config(EndpointId.BSC_V2_TESTNET)
            expect(executorConfig).to.not.be.undefined
        })
    })
})
