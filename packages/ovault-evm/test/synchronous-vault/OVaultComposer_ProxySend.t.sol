...
contract OVaultComposerProxySendTest is OVaultComposerBaseTest {
    ...
    function test_target_is_hub_reverts_when_msg_value_provided() public {
        ...
        OVaultComposerArb.depositAndSend{ value: 1 wei }(TOKENS_TO_SEND, sendParam, userA);

        vm.expectRevert(abi.encodeWithSelector(IOVaultComposer.InvalidMsgValue.selector, 0, 2));
        OVaultComposerArb.redeemAndSend{ value: 2 wei }(TOKENS_TO_SEND, sendParam, userA);
        vm.stopPrank();
    }
    ...
    function test_depositSend_target_hub() public {
        ...
        vm.startPrank(userA);
        assetOFT_arb.approve(address(OVaultComposerArb), TOKENS_TO_SEND);
        OVaultComposerArb.depositAndSend(TOKENS_TO_SEND, sendParam, userA);
        vm.stopPrank();
    ...
    function test_depositSend_target_hub_can_have_dust() public {
        ...
        OVaultComposerArb.depositAndSend(amountWithDust, sendParam, userA);
        vm.stopPrank();
    ...
    function test_redeemSend_target_hub() public {
        ...
        vm.startPrank(userA);
        oVault_arb.approve(address(OVaultComposerArb), TOKENS_TO_SEND);
        OVaultComposerArb.redeemAndSend(TOKENS_TO_SEND, sendParam, userA);
        vm.stopPrank();
    ...
    function test_redeemSend_target_hub_can_have_dust() public {
        ...
        OVaultComposerArb.redeemAndSend(amountWithDust, sendParam, userA);
        vm.stopPrank();
    ...
    function test_depositSend_target_not_hub() public {
        ...
        MessagingFee memory fee = OVaultComposerArb.quoteSend(address(assetOFT_arb), sendParam);

        vm.startPrank(userA);
        assetOFT_arb.approve(address(OVaultComposerArb), TOKENS_TO_SEND);
        OVaultComposerArb.depositAndSend{ value: fee.nativeFee }(TOKENS_TO_SEND, sendParam, userA);
        vm.stopPrank();
    ...
    function test_redeemSend_target_not_hub() public {
        ...
        MessagingFee memory fee = OVaultComposerArb.quoteSend(address(shareOFT_arb), sendParam);

        vm.startPrank(userA);
        oVault_arb.approve(address(OVaultComposerArb), TOKENS_TO_SEND);
        OVaultComposerArb.redeemAndSend{ value: fee.nativeFee }(TOKENS_TO_SEND, sendParam, userA);
        vm.stopPrank();
