...
contract OmniCounterMock is ILayerZeroComposer, OApp {
    ...
    // -------------------------------
    // Send
    function increment(uint32 _eid, uint8 _type, bytes calldata _options) external payable returns (MessagingReceipt memory receipt) {
        //        bytes memory options = combineOptions(_eid, _type, _options);
        receipt = _lzSend(_eid, MsgCodec.encode(_type, eid), _options, MessagingFee(msg.value, 0), payable(msg.sender));
