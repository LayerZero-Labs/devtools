...
import {
    createConnectionFactory as createStarknetConnectionFactory,
    createRpcUrlFactory as createStarknetRpcUrlFactory,
    createStarknetSignerFactory,
} from '@layerzerolabs/devtools-starknet'
import {
    createConnectionFactory as createSuiConnectionFactory,
    createRpcUrlFactory as createSuiRpcUrlFactory,
    createSuiSignerFactory,
} from '@layerzerolabs/devtools-sui'
...
import { getStarknetAccountFromEnv } from '../starknet/utils'
import { getSuiKeypairFromEnv } from '../sui/utils'
...
task(TASK_LZ_OAPP_WIRE)
    ...
    .setAction(async (args: Args, hre, runSuper) => {
        ...
        const solanaEnabled = solanaEid != null
        const keypair = solanaEnabled ? (await useWeb3Js()).web3JsKeypair : undefined
        const userAccount = solanaEnabled ? keypair?.publicKey : undefined
        const solanaDeployment = solanaEnabled ? getSolanaDeployment(solanaEid) : undefined
        const programId = solanaEnabled ? new PublicKey(solanaDeployment?.programId ?? '') : undefined

        if (solanaEnabled && !programId) {
        ...
        const solanaSignerFactory =
            solanaEnabled && keypair ? createSolanaSignerFactory(keypair, connectionFactory, args.multisigKey) : null
        const aptosSignerFactory = createAptosSignerFactory()
        const suiConnectionFactory = createSuiConnectionFactory(createSuiRpcUrlFactory())
        const starknetProviderFactory = createStarknetConnectionFactory(createStarknetRpcUrlFactory())
        const starknetSignerFactory = createStarknetSignerFactory(
            async (eid) => getStarknetAccountFromEnv(eid),
            starknetProviderFactory
        )
        ...
        subtask(SUBTASK_LZ_SIGN_AND_SEND, 'Sign OFT transactions', (args: SignAndSendTaskArgs, _hre, runSuper) =>
            runSuper({
                ...args,
                createSigner: firstFactory(
                    async (eid) => {
                        if (endpointIdToChainType(eid) !== ChainType.SUI) {
                            throw new Error('Not a Sui EID')
                        }
                        const keypair = getSuiKeypairFromEnv()
                        const factory = createSuiSignerFactory(keypair, suiConnectionFactory)
                        return factory(eid)
                    },
                    async (eid) => {
