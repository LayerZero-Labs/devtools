...
import { createOFTFactory as createStarknetOFTFactory } from '@layerzerolabs/ua-devtools-starknet'
import { createOFTFactory as createSuiOFTFactory } from '@layerzerolabs/ua-devtools-sui'
...
export const createSdkFactory = (
    ...
    const suiSdkFactory = createSuiOFTFactory(createSuiConnectionFactory(createSuiRpcUrlFactory()))
    const starknetSdkFactory = createStarknetOFTFactory(createStarknetConnectionFactory(createStarknetRpcUrlFactory()))
    const solanaSdkFactory =
    ...
    return async (point: OmniPoint): Promise<IOApp> => {
        if (endpointIdToChainType(point.eid) === ChainType.SOLANA) {
            if (!solanaSdkFactory) {
                throw new Error('Solana SDK factory is not configured.')
            }
            return solanaSdkFactory(point)
        } else if (endpointIdToChainType(point.eid) === ChainType.EVM) {
            return evmSdkFactory(point)
        } else if (endpointIdToChainType(point.eid) === ChainType.SUI) {
            return suiSdkFactory(point)
        } else if (endpointIdToChainType(point.eid) === ChainType.STARKNET) {
            return starknetSdkFactory(point)
        } else if (
