...
import { createConnectionFactory as createStarknetConnectionFactory } from '@layerzerolabs/devtools-starknet'
import { createConnectionFactory as createSuiConnectionFactory } from '@layerzerolabs/devtools-sui'
...
import { EndpointV2 as EndpointV2Starknet } from '@layerzerolabs/protocol-devtools-starknet'
import { EndpointV2 as EndpointV2Sui } from '@layerzerolabs/protocol-devtools-sui'
...
import { getSolanaReceiveConfig, getSolanaSendConfig, getVmReceiveConfig, getVmSendConfig } from './taskHelper'
...
const isSui = (point: OmniPoint) => endpointIdToChainType(point.eid) === ChainType.SUI
const isStarknet = (point: OmniPoint) => endpointIdToChainType(point.eid) === ChainType.STARKNET
...
const action: ActionType<TaskArgs> = async ({ logLevel = 'info', oappConfig }, hre) => {
    ...
    const tasks = graph.connections
        .filter(({ vector: { from } }) => !isSolana(from) && !isMoveVM(from) && !isSui(from) && !isStarknet(from))
    ...
    const suiTasks = graph.connections
        .filter(({ vector: { from } }) => isSui(from))
        .map(({ vector: { from, to } }) => async () => {
            const endpointV2Sdk = new EndpointV2Sui(await createSuiConnectionFactory()(from.eid), from)
            const receiveOAppConfig = await getVmReceiveConfig(endpointV2Sdk, to.eid, from.address)
            const sendOAppConfig = await getVmSendConfig(endpointV2Sdk, to.eid, from.address)
            const [sendOAppLibrary, sendOAppUlnConfig, sendOAppExecutorConfig] = sendOAppConfig ?? []
            const [receiveOAppLibrary, receiveOAppUlnConfig] = receiveOAppConfig ?? []
    ...
    const starknetTasks = graph.connections
        .filter(({ vector: { from } }) => isStarknet(from))
        .map(({ vector: { from, to } }) => async () => {
            const endpointV2Sdk = new EndpointV2Starknet(await createStarknetConnectionFactory()(from.eid), from)
            const receiveOAppConfig = await getVmReceiveConfig(endpointV2Sdk, to.eid, from.address)
            const sendOAppConfig = await getVmSendConfig(endpointV2Sdk, to.eid, from.address)
            const [sendOAppLibrary, sendOAppUlnConfig, sendOAppExecutorConfig] = sendOAppConfig ?? []
            const [receiveOAppLibrary, receiveOAppUlnConfig] = receiveOAppConfig ?? []
    ...
    await applicative(suiTasks)
    await applicative(starknetTasks)
