...
task('lz:oft:solana:create', 'Mints new SPL Token and creates new OFT Store account')
    ...
    .setAction(
        async ({
            ...
            if (isMABA) {
                console.log(
                    `Please note that for MABA mode, you must carry out the change of Mint Authority before making any cross-chain transfers. For more details: https://github.com/LayerZero-Labs/devtools/tree/main/examples/oft-main#for-oft-mint-and-burn-adapter-maba`
                )
            }
            saveSolanaDeployment(
                eid,
