...
const main = async () => {
    ...
    const programId = publicKey('HUZGfNDWQPmPXnQKLMxmHPYZae31NTz9djzeAATh5Jfp')
    const userAccount = new PublicKey('WHKCfkxo59jmFTgmQG3ZQQSjShJnBpsugSCMrtee96x')

    const deriver = new OftPDA(publicKey('HUZGfNDWQPmPXnQKLMxmHPYZae31NTz9djzeAATh5Jfp'))
    const config = deriver.config()
    const s = deriver.oftStore(publicKey('7jYGn6mZNRU78QPVciwPVLDA8RDjU5Di3Jf2ot2p5fxU'))
    const oftStore = s[0]
    ...
    // const pda = eddsa.findPda(publicKey('Fe4hzE5k82eXVa7gMVgsF76K5VZ3nTsLSwPDEJQn8dbv'), [
    ...
    const p: OmniPoint = {
        address: toWeb3JsPublicKey(oftStore).toBase58(),
        eid: 40168,
    }
    ...
    const ix = oft.initConfig(
        {
            admin: createNoopSigner(fromWeb3JsPublicKey(userAccount)),
            oftStore: oftStore,
            payer: createNoopSigner(fromWeb3JsPublicKey(userAccount)),
        },
        40161,
        {
