...
library OmniCallMsgCodecLib {
     ...
     */
    error LZ_OmniCallMsgCodecLib__InvalidDataLength(
        uint8 messageType,
        uint256 length
    );
    ...
    function encode(
        ...
        uint8 messageTypeUint = (messageType == TRANSFER_TYPE &&
            dstCall.callData.length > 0) ||
        ...
        )
    {
        ...
        if (messageType == CALL_TYPE) {
            if (data.length < MINIMAL_LENGTH_CALL) {
                revert LZ_OmniCallMsgCodecLib__InvalidDataLength(
                    messageType,
                    data.length
                );
            }
            ...
            target = address(
                uint160(
                    bytes20(
                        data[FIRST_ADDRESS_START_INDEX:FIRST_ADDRESS_END_INDEX]
                    )
                )
            );
            value = uint128(
                bytes16(data[FIRST_ADDRESS_END_INDEX:FIRST_VALUE_END_INDEX])
            );
            ...
            transferValue = uint128(
                bytes16(data[FIRST_ADDRESS_END_INDEX:FIRST_VALUE_END_INDEX])
            );
            target = address(
                uint160(
                    bytes20(
                        data[FIRST_VALUE_END_INDEX:SECOND_ADDRESS_END_INDEX]
                    )
                )
            );
            value = uint128(
                bytes16(data[SECOND_ADDRESS_END_INDEX:SECOND_VALUE_END_INDEX])
            );
    ...
    function isCallType(bytes calldata data) internal pure returns (bool) {
        return
            uint8(data[0]) == CALL_TYPE && data.length >= MINIMAL_LENGTH_CALL;
    }
     ...
     */
    function isCallAndTransferType(
        bytes calldata data
    ) internal pure returns (bool) {
        return
            uint8(data[0]) == CALL_AND_TRANSFER_TYPE &&
