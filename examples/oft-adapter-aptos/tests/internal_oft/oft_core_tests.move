...
module oft::oft_core_tests {
    ...
    use oft::oft_core::{Self, oft_v1_compatibility_mode_set, SEND, SEND_AND_CALL, set_v1_compatibility_mode};
    use oft::oft_store;
    use oft_common::oft_compose_msg_codec;
    use oft_common::oft_msg_codec;
    use oft_common::oft_v1_msg_codec::{Self, PT_SEND, PT_SEND_AND_CALL};
    ...
    fun test_send() {
        ...
        // Turn on and off v1 compatibility mode (to test turning off works)
        set_v1_compatibility_mode(DST_EID, true);
        set_v1_compatibility_mode(DST_EID, false);
    ...
    #[test]
    fun test_send_v1_compatibility() {
        setup(SRC_EID, DST_EID);
        ...
        // Compose message not allowed in v1 compatibility mode
        ...
        set_v1_compatibility_mode(DST_EID, true);
        assert!(was_event_emitted(&oft_v1_compatibility_mode_set(DST_EID, true)), 1);
        ...
        let (messaging_receipt, amount_sent_ld, amount_received_ld) = oft_core::send(
            ...
            |message, options| {
                called_send = true;
                assert!(!oft_v1_msg_codec::has_compose(&message), 0);
                assert!(oft_v1_msg_codec::send_to(&message) == to, 0);
                assert!(oft_v1_msg_codec::amount_sd(&message) == 40, 1);
                assert!(options == b"options", 2);
    ...
    fun test_receive_v1() {
        setup(DST_EID, SRC_EID);
        set_v1_compatibility_mode(SRC_EID, true);
        ...
        let message = oft_v1_msg_codec::encode(
            PT_SEND(),
            bytes32::from_address(@0x2000),
            123,
            bytes32::from_address(@0x3000),
    ...
    #[test]
    fun test_receive_with_compose_v1() {
        setup(DST_EID, SRC_EID);
