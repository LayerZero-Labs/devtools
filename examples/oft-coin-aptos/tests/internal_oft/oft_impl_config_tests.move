...
module oft::oft_impl_config_tests {
    ...
    use oft::oft_impl_config::{
        ...
        get_fee_bps,
        has_rate_limit,
        in_flight_at_timestamp,
        rate_limit_capacity_at_timestamp,
        rate_limit_config,
        rate_limit_set_event,
        rate_limit_updated_event,
        redirect_to_admin_if_blocklisted,
        release_rate_limit_capacity,
        set_blocklist, set_fee_bps, try_consume_rate_limit_capacity_at_timestamp, unset_rate_limit,
    };
    use oft::oft_store;
    ...
    fun set_rate_limit() {
        ...
        assert!(in_flight_at_timestamp(30100, 10) == 0, 1);
        assert!(rate_limit_capacity_at_timestamp(30100, 10) == MAX_U64, 2);
        ...
        assert!(in_flight_at_timestamp(30100, 100) == 0, 1);
        assert!(rate_limit_capacity_at_timestamp(30100, 100) == 20000, 2);

        // 100 seconds later
        assert!(in_flight_at_timestamp(30100, 200) == 0, 1);
        assert!(rate_limit_capacity_at_timestamp(30100, 200) == 20000, 2);

        // consume 10% of the capacity
        try_consume_rate_limit_capacity_at_timestamp(30100, 2000, 200);
        assert!(in_flight_at_timestamp(30100, 200) == 2000, 1);
        assert!(rate_limit_capacity_at_timestamp(30100, 200) == 18000, 2);

        // 10 seconds later: in flight should decline by 20000/1000s * 10s = 200
        assert!(in_flight_at_timestamp(30100, 210) == 1800, 1);
        assert!(rate_limit_capacity_at_timestamp(30100, 210) == 18200, 2);

        // 20 seconds later: in flight should decline by 20000/1000s * 20s = 400
        assert!(in_flight_at_timestamp(30100, 220) == 1600, 1);
        assert!(rate_limit_capacity_at_timestamp(30100, 220) == 18400, 2);
        ...
        assert!(in_flight_at_timestamp(30100, 220) == 1600, 1);
        assert!(rate_limit_capacity_at_timestamp(30100, 220) == 28400, 2);

        // 10 seconds later: in flight should decline by 30000/1000s * 10s = 300
        assert!(in_flight_at_timestamp(30100, 230) == 1300, 1);
        assert!(rate_limit_capacity_at_timestamp(30100, 230) == 28700, 2);

        // 10 seconds later: in flight should decline by 30000/1000s * 10s = 300
        assert!(in_flight_at_timestamp(30100, 240) == 1000, 1);
        assert!(rate_limit_capacity_at_timestamp(30100, 240) == 29000, 2);

        // 100 seconds later: in flight should decline fully (without overshooting): 30000/1000s * 100s = 3000
        assert!(in_flight_at_timestamp(30100, 300) == 0, 1);
        assert!(rate_limit_capacity_at_timestamp(30100, 300) == 30000, 2);

        // Consume again
        try_consume_rate_limit_capacity_at_timestamp(30100, 2000, 300);
        try_consume_rate_limit_capacity_at_timestamp(30100, 2000, 310);
        // 2000 + (2000 - 30000/1000*10) = 3800
        assert!(in_flight_at_timestamp(30100, 310) == 3700, 1);
        ...
        let (limit, window) = rate_limit_config(30100);
        assert!(limit == 0 && window == 0, 1);
        assert!(in_flight_at_timestamp(30100, 320) == 0, 1);
        assert!(rate_limit_capacity_at_timestamp(30100, 320) == MAX_U64, 2);
    }

    #[test]
    fun set_rate_limit_net() {
        setup();
        ...
        try_consume_rate_limit_capacity_at_timestamp(30100, 20000, 0);
        assert!(in_flight_at_timestamp(30100, 0) == 20000, 1);
        assert!(rate_limit_capacity_at_timestamp(30100, 0) == 0, 2);

        // 50 seconds later: in flight should decline by 20000/1000s * 500s = 10000
        assert!(in_flight_at_timestamp(30100, 500) == 10000, 1);
        assert!(rate_limit_capacity_at_timestamp(30100, 500) == 10000, 2);

        // release most of remaining capacity
        release_rate_limit_capacity(30100, 9000);
        assert!(in_flight_at_timestamp(30100, 500) == 1000, 1);
        assert!(rate_limit_capacity_at_timestamp(30100, 500) == 19000, 2);

        // consume all of remaining capacity
        try_consume_rate_limit_capacity_at_timestamp(30100, 19000, 500);
        assert!(in_flight_at_timestamp(30100, 500) == 20000, 1);
        assert!(rate_limit_capacity_at_timestamp(30100, 500) == 0, 2);

        // release excess capacity (5x limit) - should not overshoot
        release_rate_limit_capacity(30100, 100_000);
        assert!(in_flight_at_timestamp(30100, 500) == 0, 1);
        assert!(rate_limit_capacity_at_timestamp(30100, 500) == 20000, 2);
    }
}
