...
const contractName_composer = 'WrappedHyperCoreAdapter'
...
const deploy: DeployFunction = async (hre) => {
    ...
    if (!hip1Token.txData.weiDiff) {
        throw new Error('Wei diff is not set for the native spot')
    }
    ...
    const { address: address_composer } = await deploy(contractName_composer, {
        ...
        args: [
            ...
            hip1Token.txData.weiDiff, // Wei diff
        ],
        log: true,
    ...
    console.log(
        `Deployed WrappedHyperCoreAdapter contract: ${contractName_composer}, network: ${hre.network.name}, address: ${address_composer}`
    )
...
// WrappedHyperCoreAdapter-hyperliquid
deploy.tags = [`${contractName_composer}-hyperliquid`]
