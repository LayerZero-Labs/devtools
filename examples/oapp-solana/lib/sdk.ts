...
export class CustomOAppSDK extends OmniSDK implements IOApp {
    ...
    async getEnforcedOptions(eid: EndpointId, msgType: number): Promise<Bytes> {
        ...
        return toHex(new Uint8Array(0)) // since right now counter does not have enforced options
        try {
            // currently counter does not have getEnforcedOptions
    ...
    protected async _setOAppAdminIx(address: OmniAddress) {
        return this._setOAppConfigIx({ __kind: 'Admin', admin: publicKey(address) })
    }

    protected async _setOFTDelegateIx(address: OmniAddress) {
        return this._setOAppConfigIx({ __kind: 'Delegate', delegate: publicKey(address) })
    }
    ...
    protected async _setPeerFeeBpsIx(eid: EndpointId, feeBps = 0) {
