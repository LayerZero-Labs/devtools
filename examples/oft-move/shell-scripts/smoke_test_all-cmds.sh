...
echo -e "y\ny" | pnpm run lz:sdk:move:build --lz-config move.layerzero.config.ts --named-addresses oft=$ACCOUNT_ADDRESS,oft_admin=$ACCOUNT_ADDRESS

# Deploy contracts
echo "Deploying contracts..."
echo -e "y\ny" | pnpm run lz:sdk:move:deploy --lz-config move.layerzero.config.ts --address-name oft --named-addresses oft=$ACCOUNT_ADDRESS,oft_admin=$ACCOUNT_ADDRESS --move-deploy-script deploy-move/OFTInitParams.ts
...
echo -e "y\ny" | pnpm run lz:sdk:evm:wire --lz-config move.layerzero.config.ts
echo -e "y\ny" | pnpm run lz:sdk:move:wire --lz-config move.layerzero.config.ts

# Set fee (example with EID 2)
echo "Setting fee..."
echo -e "y\ny" | pnpm run lz:sdk:move:set-fee --lz-config move.layerzero.config.ts --fee-bps 1000 --to-eid 40102

# Set rate limit (example with EID 2)
echo "Setting rate limit..."
echo -e "y\ny" | pnpm run lz:sdk:move:set-rate-limit --lz-config move.layerzero.config.ts --rate-limit 10000 --window-seconds 60 --to-eid 40102

# Unset rate limit (example with EID 2)
echo "Unsetting rate limit..."
echo -e "y\ny" | pnpm run lz:sdk:move:unset-rate-limit --lz-config move.layerzero.config.ts --to-eid 40102
