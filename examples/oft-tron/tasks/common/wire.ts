...
interface Args {
    oappConfig: string
    privateKey: string
    isTronInitConfig?: boolean
}

export default async function (args: Args, hre: HardhatRuntimeEnvironment) {
    const { oappConfig, privateKey, isTronInitConfig = false } = args
    ...
    // Initialize TronWeb
    const tronWeb = initTronWeb(tronEid === EndpointId.TRON_V2_MAINNET ? 'mainnet' : 'testnet', privateKey)
