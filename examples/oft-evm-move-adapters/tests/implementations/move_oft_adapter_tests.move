...
#[test_only]
module oft::move_oft_adapter_tests {
    ...
    fun setup(): MintRef {
        ...
        move_oft_adapter::init_module_for_test();
    ...
    fun test_debit() {
        ...
        let (sent, received) = move_oft_adapter::debit_fungible_asset(
            @444,
            &mut fa,
            min_amount_ld,
            dst_eid,
        ...
        let balance = primary_fungible_store::balance(escrow_address(), move_oft_adapter::metadata());
    ...
    fun test_credit() {
        ...
        move_oft_adapter::debit_fungible_asset(
            @444,
            &mut deposit,
            0,
            src_eid,
        ...
        let balance = primary_fungible_store::balance(to, move_oft_adapter::metadata());
        assert!(balance == 0, 0);

        let credited = move_oft_adapter::credit(
            to,
            amount_ld,
            src_eid,
            lz_receive_value,
    ...
    fun test_debit_view() {
        ...
        let (sent, received) = move_oft_adapter::debit_view(123456700, 100, 2);
        assert!(sent == 123456700, 0);
        assert!(received == 123456700, 0);
    }
    ...
    fun test_debit_view_fails_if_less_than_min() {
        ...
        move_oft_adapter::debit_view(32, 100, 2);
    }
    ...
    fun test_build_options() {
        ...
        let options = move_oft_adapter::build_options(
            message_type,
            dst_eid,
    ...
    fun test_inspect_message() {
        ...
        move_oft_adapter::inspect_message(
            &x"1234",
            &x"1234",
            true,
        );
    ...
    fun test_oft_limit_and_fees() {
        ...
        initialize_native_token_for_test();
        let (limit, fees) = move_oft_adapter::oft_limit_and_fees(
            123,
            x"1234",
            123,
            100,
    ...
    fun test_set_fee_bps() {
        ...
        let (oft_limit, oft_fee_details) = move_oft_adapter::oft_limit_and_fees(
            123,
            x"1234",
            100_000_000,
            100,
        ...
        let fee_deposited = primary_fungible_store::balance(deposit_address, move_oft_adapter::metadata());
    ...
    fun test_set_blocklist_credit() {
        ...
        move_oft_adapter::set_blocklist(
            admin,
            blocklisted_address,
            true,
        );
        ...
        let admin_balance = primary_fungible_store::balance(@oft_admin, move_oft_adapter::metadata());
        ...
        let to_balance = primary_fungible_store::balance(blocklisted_address, move_oft_adapter::metadata());
    ...
    fun test_disable_blocklist() {
        ...
        let admin = &create_signer_for_test(@oft_admin);
        move_oft_adapter::irrevocably_disable_blocklist(admin);
    ...
    fun test_rate_limit() {
        ...
        let (limit, window) = move_oft_adapter::rate_limit_config(30100);
        assert!(limit == 0 && window == 0, 0);

        let admin = &create_signer_for_test(@oft_admin);
        move_oft_adapter::set_rate_limit(admin, 30100, 2500, 100);
        assert!(was_event_emitted(&oft_impl_config::rate_limit_set_event(30100, 2500, 100)), 1);

        let (limit, window) = move_oft_adapter::rate_limit_config(30100);
        assert!(limit == 2500 && window == 100, 1);

        let (oft_limit, fee_detail) = move_oft_adapter::oft_limit_and_fees(
            30100,
            x"1234",
            123,
            100,
